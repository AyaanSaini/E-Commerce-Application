	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException {
		...
		if(session.getAttribute("session_user_role") == null){
			...
			if(id != null){
				...
				try {
					res = productService.deleteProduct(productId);
					if(res == 1){
						message = "Product Deleted Successfully";
						response.sendRedirect("/Git_Punjabi_Fashion/jsp/admin/deleteproduct.jsp?message=" + URLEncoder.encode(message, "UTF-8"));
					}else if(res == 2){
						message = "Product Not Found";
						response.sendRedirect("/Git_Punjabi_Fashion/jsp/admin/deleteproduct.jsp?error=" + URLEncoder.encode(message, "UTF-8"));
					}
					else{
						message = "Product Not Deleted, Contact to Syatem Admin";
						response.sendRedirect("/Git_Punjabi_Fashion/jsp/admin/deleteproduct.jsp?error=" + URLEncoder.encode(message, "UTF-8"));
					}
